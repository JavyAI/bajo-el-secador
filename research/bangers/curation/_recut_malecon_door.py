#!/usr/bin/env python3
"""Malecón recut: couple-walk doors, 100 + 100, no neighbor doorway."""
...
# Locked #1. Forgotten inside 15, not the throne. Max 2 of a lead.
# Door = couple walking. Not barbershop, not dryer funeral, not Galería porch.
AYER15 = [
...
# High couple tail first. Still bangers. Not the throne.
AYER_TAIL = [
    ...
    ("0XCot42qTvA", "Aventura", "Dile al Amor", 2009, "Aventura", 280000000),
    ("CsziEW_gYhU", "Aventura", "Los Infieles", 2006, "Aventura", 200000000),
    ("SX6dWnfKky8", "Aventura", "Déjà Vu", 2009, "Aventura", 90000000),
    ("Eaie5S-zFcE", "Luis Miguel", "Inolvidable", 1991, "Warner Music México", 40000000),
...
HOY_TAIL = [
    ("_kxz7WX4mLU", "Cali Y El Dandee, Juan Magán, Sebastián Yatra", "Por Fin Te Encontré", 2015, "CaliyElDandeeVEVO", 1300000000),
    ("Geqmpq0tjNU", "Carlos Vives, Marc Anthony", "Cuando Nos Volvamos a Encontrar", 2014, "CarlosVivesVEVO", 890000000),
    ("KIBeny5wq6M", "Carlos Vives, Ricky Martin", "Canción Bonita", 2021, "CarlosVivesVEVO", 312000000),
    ...
    ("K8q5boZdKuU", "David Bisbal", "Diez Mil Maneras", 2014, "DavidBisbalVEVO", 44700000),
...
def interleave(head: list, tail: list) -> list:
    ...
    out = list(head)
    remaining = list(tail)
    while remaining and len(out) < 100:
        ...
        if picked is None:
            raise RuntimeError(
                f"stuck n={len(out)} prev={prev} left={[lead(t['artist']) + ':' + t['id'] for t in remaining[:12]]}"
            )
...
def build(era: str, first15, tail) -> list[dict]:
    ...
    for row in first15:
        t = to_track(row, True)
        assert len(t["id"]) == 11, t["id"]
        assert t["id"] not in used
        used.add(t["id"])
        head.append(t)
    rest: list[dict] = []
    for row in tail:
        ...
        if t["id"] in used:
            continue
        if len(t["id"]) != 11:
            raise SystemExit(f"bad id {t['id']}")
        if era == "ayer" and not (1980 <= t["year"] <= 2010):
            raise SystemExit(f"AYER year {t}")
        if era == "hoy" and t["year"] < 2011:
            raise SystemExit(f"HOY year {t}")
        used.add(t["id"])
        rest.append(t)
    tracks = interleave(head, rest)
    if len(tracks) < 100:
        raise SystemExit(f"{era} only {len(tracks)}")
    return tracks[:100]


def hygiene(tracks: list[dict], era: str, locked: str, forgotten: str) -> None:
    ...
    if era == "ayer":
        assert 1980 <= min(years) and max(years) <= 2010, (min(years), max(years))
    else:
        assert min(years) >= 2011, min(years)
    print(f"{era} hygiene OK years {min(years)}-{max(years)}")
    print("  leads", counts.most_common(10))
...
def art_ok(vid: str) -> tuple[bool, int]:
    local = ROOT / "assets" / "covers" / f"{vid}.jpg"
    ...
    try:
        with urllib.request.urlopen(req, context=CTX, timeout=15) as r:
            payload = r.read()
        return r.status == 200 and len(payload) > 2000, len(payload)
    except Exception:
        return False, 0
...
def verify_door(tracks: list[dict], era: str) -> None:
    ...
    for t in tracks[:15]:
        ...
        mark = "OK" if ok and art else "FAIL"
        print(f"  {mark} {era} {t['id']} {t['title']} -> {author} | {title[:70]} art={nbytes}")
        if not ok:
            fails.append(t["id"])
        if not art:
...
def neighbor_overlap(tracks: list[dict], era_dir: str) -> None:
    ids = {t["id"] for t in tracks[:15]}
    for room in ["silla", "secador", "colmado", "galeria", "limpieza"]:
        pub = json.loads((ROOT / "public" / era_dir / f"{room}.json").read_text())
        theirs = [t["id"] for t in pub["tracks"][:15]]
        hit = [i for i in theirs if i in ids]
        titles = []
        by_id = {t["id"]: t["title"] for t in tracks[:15]}
        titles = [by_id[i] for i in hit]
        print(f"  vs {room}: {len(hit)} {titles}")


def dump(era: str, tracks: list[dict]) -> None:
    path = CUR / f"malecon-{era}.json"
    path.write_text(
        json.dumps(
            {"room": "malecon", "era": era, "count": 100, "introCount": 15, "tracks": tracks},
            ensure_ascii=False,
            indent=2,
        )
        + "\n"
...
def main() -> None:
    ...
    hygiene(ayer, "ayer", "v0ckuv1xBm0", "uBwk106e3es")
    hygiene(hoy, "hoy", "QFs3PIZb3js", "bdOXnTbyk0g")
    ...
    neighbor_overlap(ayer, "ayer")
    neighbor_overlap(hoy, "hoy")
