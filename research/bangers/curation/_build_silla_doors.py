#!/usr/bin/env python3
"""Barbería (silla) AYER + HOY. Doorway first. Official 11-char only."""
...
def consec(tracks: list) -> int:
    return sum(1 for i in range(1, len(tracks)) if lead(tracks[i]["artist"]) == lead(tracks[i - 1]["artist"]))


# ---------------------------------------------------------------------------
# AYER — típica first, then Aventura-era. Authored #1 Obsesión stays.
# Forgotten #1 Un Beso at #3. Medicina + Corazón Culpable + Dime Qué Faltó
# for wow. Aventura 12. No Burbujas. No Toby. Ella y Yo tail only.
# ---------------------------------------------------------------------------
AYER_15 = [
...
# La Asesina (393M) and Imitadora (797M) raise the wow.
# Cut Volví, Deja Vu, Carita, 2002 Kiko padding.
# ---------------------------------------------------------------------------
HOY_15 = [
...
HOY_TAIL = [
    ...
    ("0w1eAAmOU4k", "Antony Santos, Romeo Santos", "Bellas", 2019, "RomeoSantosVEVO", 0),
    ...
    ("kVtfXd_WdkA", "Prince Royce", "Rechazame", 2012, "Prince Royce", 142100000),
    ...
    ("COh7Al7hpPA", "Luis Segura, Romeo Santos", "Como Yo", 2019, "RomeoSantosVEVO", 0),
    ...
    ("yUu6bxxRUGI", "Zacarías Ferreira", "Todos Juntos", 2019, "Zacarias Ferreira Oficial", 0),
    ...
    ("uXLkI5Lknpo", "Grupo Extra", "Dile a Él", 2014, "Grupo Extra", 0),
...
def hygiene(tracks: list, era: str) -> None:
    ...
    for i, t in enumerate(tracks[:15], 1):
        print(f"  {i:2}. {t['artist'].split(',')[0]} — {t['title']} ({t.get('year')})")
    ...
    if dups:
        print("DUP IDS", dups)
    bad = [t["id"] for t in tracks if not ID_RE.match(t["id"])]
    if bad:
        print("BAD IDS", bad)
    over12 = [(k, v) for k, v in Counter(lead(t["artist"]) for t in tracks).items() if v > 12]
    over18 = [(k, v) for k, v in over12 if v > 18]
    print("over12", over12, "over18", over18)
...
def main() -> None:
    ...
    romeo_in_15 = sum(1 for r in hoy_15 if lead(r[1]) == "romeo santos")
    romeo_budget = max(0, 18 - romeo_in_15)
    trimmed = []
    romeo_kept = 0
    for r in hoy_tail:
        if lead(r[1]) == "romeo santos":
            if romeo_kept >= romeo_budget:
                print(" ROMEO CAP skip", r[2])
                continue
            romeo_kept += 1
        trimmed.append(r)
    hoy_tail = trimmed
